  ...
  render() {
    return (
      <React.Fragment>
        ...
        <ul>
          player1 event card in hand: {this.state.player1.event.toString()}{' '}
          turn: {this.state.player1.turn.toString()}
          {this.state.player1.hand.map(card => (
            <li key={card.title}>{card.title}</li>
          ))}
        </ul>
        <ul>
          player2 event card in hand: {this.state.player2.event.toString()}{' '}
          turn: {this.state.player2.turn.toString()}
          {this.state.player2.hand.map(card => (
            <li key={card.title}>{card.title}</li>
          ))}
        </ul>
        <ul>
          player3 event card in hand: {this.state.player3.event.toString()}{' '}
          turn: {this.state.player3.turn.toString()}
          {this.state.player3.hand.map(card => (
            <li key={card.title}>{card.title}</li>
          ))}
        </ul>
        <ul>
          player4 event card in hand: {this.state.player4.event.toString()}{' '}
          turn: {this.state.player4.turn.toString()}
